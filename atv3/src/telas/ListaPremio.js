...
const handleOkDelete = async () => {
    const url = "http://localhost:3000/premio/" + idSelecionado;
    const param = {
        method: 'DELETE',
    ...
    if (respostaJson.status === 200){
        message.success('Prêmio excluído com sucesso!');
    }else if (respostaJson.status !== 200){
        message.error('Erro ao excluir o prêmio');
    }
...
const handleOkEdit = async (descricao,pontos, quantidade ) => {
    ...
    if (respostaJson.status === 200){
        ...
        message.error('Erro ao editar o prêmio');
    }
