...
const handleOkAdd = async () =>{
    const url = "http://localhost:3000/premio/" + idSelecionado +"/usuario/" + id;
    const param = {
        method: 'POST',
    ...
    const resposta = await fetch(url, param);
        ...
        if (respostaJson.status === 200){
            message.success('O prêmio foi atribuído com sucesso!');
        }else if (respostaJson.status !== 200){
            message.error('Este usuário já recebeu este prêmio');
        ...
        <div >
            ...
            <Modal
                ...
                onCancel={handleCancel}>
                    <p>Deseja realmente atribuir este prêmio a este usuário?</p>
            </Modal>
       </div>
    </div>
}
